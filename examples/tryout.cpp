...
int main() {
    ...
    p.vecprobs << 0.3, 0.7, 0.1, 0.7, 0.2, 0.9, 0.1, 0.3, 0.5, 0.2;
    ...
    data.x << 1.0, 0.5, -0.2, 1.0, 1.0, 0.0, 1.0, 0.0, 0.8;
    ...
    Eigen::MatrixXd new_prior = pypolca::compute_prior_from_beta(data.x, beta, n_classes);
    ...
    auto [grad, info] =
        pypolca::compute_beta_derivatives(data, posterior, new_prior, beta, n_classes);
    ...
    auto [new_beta, updated_prior] =
        pypolca::update_beta(data, posterior, new_prior, beta, n_classes);
