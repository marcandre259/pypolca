...
PYBIND11_MODULE(_core, m) {
  ...
  py::class_<pypolca::Data>(m, "Data")
      .def(py::init<>())
      .def_readwrite("y", &pypolca::Data::y,
                     "N x J response matrix (1-based, 0=missing)")
  ...
  py::class_<pypolca::Results>(m, "Results")
      ...
      .def_readonly("posterior", &pypolca::Results::posterior,
                    "N x nclass posterior probs")
  ...
  // --- fit_em ---
  m.def("fit_em", &pypolca::fit_em, py::arg("data"), py::arg("nclass"),
        py::arg("maxiter") = 1000, py::arg("tol") = 1e-10,
  ...
  m.def("compute_ylik", &pypolca::compute_log_ylik, py::arg("data"),
        py::arg("params"), py::arg("nclass"),
        "Compute class-conditional likelihoods (N x nclass).");

  m.def("e_step", &pypolca::e_step, py::arg("data"), py::arg("params"),
        py::arg("prior"), py::arg("nclass"),
        "E-step: compute posterior class membership probabilities and "
        "log-likelihood. Returns (posterior, loglik).");

  m.def("m_step_probs", &pypolca::m_step_probs, py::arg("data"),
        py::arg("posterior"), py::arg("num_choices"), py::arg("nclass"),
        "M-step: update class-conditional response probabilities.");

  m.def("compute_beta_derivatives", &pypolca::compute_beta_derivatives,
        py::arg("data"), py::arg("posterior"), py::arg("prior"),
        py::arg("beta"), py::arg("nclass"),
        "Compute gradient and observed information for Newton-Raphson.");

  m.def("update_beta", &pypolca::update_beta, py::arg("data"),
        py::arg("posterior"), py::arg("prior"), py::arg("beta"),
        py::arg("nclass"),
        "Single Newton-Raphson step for beta. Returns (new_beta, new_prior).");

  m.def("compute_prior_from_beta", &pypolca::compute_prior_from_beta,
