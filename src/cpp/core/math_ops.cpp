...
static int prob_index(int k, int j, int r, const std::vector<int> &num_choices, int n_choices) {
    int pos = 0;
...
Eigen::MatrixXd compute_prior_from_beta(const Eigen::MatrixXd &x, const Eigen::VectorXd &beta,
    ...
    if (beta.size() != (nclass - 1) * M) {
        throw std::invalid_argument("beta size must equal (nclass - 1) * n_covariates");
    }
            ...
            for (int r = 1; r < nclass; r++) {
                ...
                for (int l = 0; l < M; l++) {
                    ...
                    for (int s = 1; s < r; s++) {
                        ...
                        hessians(row, col2) +=
                            x(i, m) * x(i, l) *
                            (-1.0 * prior(i, r) * prior(i, s) + posterior(i, r) * posterior(i, s));
                    }
                }
            }
        }
...
SEs compute_standard_errors(const Data &data, const Params &params,
    ...
    for (int r = 0; r < R; r++) {
        for (int j = 0; j < J; j++) {
            ...
            Eigen::MatrixXd J_sub = Eigen::MatrixXd(p.asDiagonal()) - (p * p.transpose());
    ...
    if (R > 1) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            Eigen::MatrixXd psi = Eigen::MatrixXd(pi.asDiagonal()) - pi * pi.transpose();
            for (int r = 1; r < R; r++) {
                for (int l = 0; l < S; l++) {
