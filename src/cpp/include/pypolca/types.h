 ...
 *   num_choices:  Length-J vector; num_choices[j] = number of categories for item j.
 */
struct Data {
    Eigen::MatrixXi y;
    Eigen::MatrixXd x;
 ...
 *              Layout: for each class r, then each item j, then each category k.
 *              Total length = nclass * sum_j(num_choices[j]).
 *   beta:      Flattened covariate coefficients.
 *              Layout: column-major S x (nclass-1) matrix stored as vector.
 */
struct Params {
...
struct Results {
    ...
    // std::vector<Eigen::MatrixXd> probs_se;  // J matrices of size nclass x K_j
